...
class opParams:
  def __init__(self):
    ...
    self.fork_params = {'camera_offset': Param(0.06, NUMBER, 'Your camera offset to use in lane_planner.py', live=True),
                        ...
                        'fast_upload': Param(False, bool, 'If True, openpilot won\'t wait between uploading files'),
                        'steer_fault_fix': Param(False, bool, live=True),
                        ...
                        '0_coast_accel': Param(0.18, NUMBER, 'coast accel threshold at 0 (default: 0.538)', live=True),
                        'apply_accel': Param(None, NONE_OR_NUMBER, live=True),
                        'apply_gas': Param(None, NONE_OR_NUMBER, live=True),
                        'lat_p': Param(.08, NUMBER, live=True),
                        'lat_i': Param(.005, NUMBER, live=True),
                        ...
                        'coast_smoother': Param(True, bool, 'Smoothly ramps up gas output across the coasting accel at that speed', live=True),
                        'standstill_accel_multiplier': Param(3, NUMBER, 'Multiplier for accel when at a standstill to resume quicker. 1 disables this', live=True),
                        'coast_spread': Param(0.2, NUMBER, '.08 is default but 0.16 works nicely', live=True),
                        'apply_pitch_mod': Param(False, bool, 'To apply pitch mod', live=True),
                        'pitch_min_speed': Param(1, NUMBER, 'Min speed to apply pitch mod (mph)', live=True),
                        ...
                        'pitch_angle_v': Param(1.2, NUMBER, 'multiplier for inclines. for declines, it will (1 - this value + 1) ex. 1.1 is 0.9', live=True),
                        'pitch_deadzone': Param(1, NUMBER, 'In degrees, the deadzone to not apply pitch mod', live=True),
                        'accel_time_constant_0_mph': Param(.25, NUMBER, 'Time constant for eager accel (Approaching 0 is no change)', live=True),
                        'accel_time_constant_45_mph': Param(.25, NUMBER, 'Time constant for eager accel (Approaching 0 is no change)', live=True),
                        'accel_time_constant_70_mph': Param(.25, NUMBER, 'Time constant for eager accel (Approaching 0 is no change)', live=True),
                        'accel_eagerness': Param(1, NUMBER, 'Multiplier for change in accel to make it eager (1 is still eager)', live=True),
