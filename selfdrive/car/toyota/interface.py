...
class CarInterface(CarInterfaceBase):
  ...
  def get_params(candidate, fingerprint=gen_empty_fingerprint(), has_relay=False, car_fw=[]):  # pylint: disable=dangerous-default-value
    ...
    if candidate == CAR.PRIUS:
      ...
      ret.lateralTuning.pid.kpBP, ret.lateralTuning.pid.kpV = [[20, 31], [0.045, 0.025]]  # 45 to 70 mph
      ret.lateralTuning.pid.kiBP, ret.lateralTuning.pid.kiV = [[20, 31], [0.005, 0.002]]
    ...
    ret.gasMaxBP = [0.]
    ret.gasMaxV = [0.5]
    ret.longitudinalTuning.kpV = [kp / 1.5 for kp in [3.6, 2.4, 1.5]]  # slightly slower loop for all cars
    ret.longitudinalTuning.kiV = [ki / 1.5 for ki in [0.54, 0.36]]
