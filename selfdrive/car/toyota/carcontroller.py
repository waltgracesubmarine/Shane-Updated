...
class CarController:
  ...
  def update(self, CC, CS):
    ...
    if not CC.latActive or CS.steer_state in (9, 25) or abs(CS.out.steeringRateDeg) > 100:
      apply_steer = 0
      apply_steer_req = 0
    else:
      apply_steer_req = 1
    ...
    if CS.out.standstill and not self.last_standstill and self.CP.carFingerprint not in NO_STOP_TIMER_CAR and not self.standstill_hack:
